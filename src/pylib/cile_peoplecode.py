...
"""A Code Intelligence Language Engine for the PeopleCode language.
...
Module Usage:
    from cile_peoplecode import scan
    mtime = os.stat("bar.peoplecode")[stat.ST_MTIME]
    content = open("bar.peoplecode", "r").read()
    scan(content, "bar.peoplecode", mtime=mtime)
"""
...
class PeopleCodeCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.peoplecode")
...
def scan_buf(buf, mtime=None, lang="PeopleCode"):
    """Scan the given PeopleCodeBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {PeopleCodeBuffer} is the PeopleCode buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the PeopleCode CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this PeopleCode content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l PeopleCode <example-PeopleCode-file>
    ...
    # Dev Note:
    # This is where you process the PeopleCode content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-PeopleCode-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for PeopleCode content, you may
    #   want to use it.
