# Registers the PeopleCode language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koPeopleCodeLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language PeopleCode")
    registry.registerLanguage(KoPeopleCodeLanguage())

class KoPeopleCodeLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "PeopleCode"
    lexresLangName = "PeopleCode"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "c7dc3cba-adcc-41e9-81a9-3dbef280adca"
    defaultExtension = '.pc'
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'PeopleCode'}
